...
class FavouritesController extends GetxController {
  ...
  Future<void> initFavorites() async {
    final json = TLocalStorage.instance().readData('ұнаған тауарлар');
    if (json != null) {
  ...
  void toggleFavoriteProduct(String productId) {
    ...
    if (!favorites.containsKey(productId)) {
      ...
      TLoaders.customToast(message: 'Өнім ұнамдылар тізіміне қосылды.');
    } else {
      TLocalStorage.instance().removeData(productId);
      favorites.remove(productId);
      saveFavoritesToStorage();
      favorites.refresh();
      TLoaders.customToast(message: 'Өнім ұнамдылар тізімінен алынды.');
    }
  }
  ...
  void saveFavoritesToStorage() {
    ...
    TLocalStorage.instance().writeData('ұнаған тауарлар', encodedFavorites);
  }
