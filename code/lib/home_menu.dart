    ...
    return Scaffold(
      ...
      bottomNavigationBar: Obx(
        () => NavigationBar(
          ...
          elevation: 0,
          indicatorColor: THelperFunctions.isDarkMode(context) ? TColors.white.withOpacity(0) : TColors.orange.withOpacity(0.8),
          onDestinationSelected: (index) => controller.selectedMenu.value = index,
          destinations: const [
            NavigationDestination(icon: Icon(Iconsax.home), label: 'Басты бет'),
            NavigationDestination(icon: Icon(Iconsax.shop), label: 'Дүкен'),
            NavigationDestination(icon: Icon(Iconsax.heart), label: 'Таңдаулылар'),
